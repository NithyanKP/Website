...
!(function($) {
  ...
  $(".research-details-carousel").owlCarousel({
    autoplay: true,
    dots: true,
    loop: true,
    items: 1
        ...
        slide: function( event, ui ) {
          ...
          portfolioIsotope.isotope({
            filter: function(){
              ...
              var filter = true;
              if ($('.filter-active').data('filter') == '*'){
                filter = true;
                }
                else{
                  filter = $(this).hasClass($('.filter-active').data('filter').replace('.',''));
                }
              return filter  && (parseInt(number) <= ui.values[ 1 ]) && (ui.values[ 0 ] <= parseInt(number)) ;
            }
          });
        }
      });
